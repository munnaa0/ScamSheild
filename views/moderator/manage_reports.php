...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'approve') {
        $query = "UPDATE scam_reports SET status='verified', reviewed_by=$moderator_id, 
    ...
    if ($action == 'investigate') {
        $query = "UPDATE scam_reports SET status='investigating', reviewed_by=$moderator_id, 
                  reviewed_at=NOW() WHERE report_id=$report_id";
        if (mysqli_query($conn, $query)) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'reject_report') {
        ...
        $query = "UPDATE scam_reports SET status='rejected', rejection_reason='$rejection_reason', 
                  reviewed_by=$moderator_id, reviewed_at=NOW() WHERE report_id=$report_id";
...
if ($view_id) {
    $query = "SELECT sr.*, sc.category_name, u.username as reporter_username, u.email as reporter_user_email,
              moderator.username as reviewed_by_username
              FROM scam_reports sr 
              LEFT JOIN scam_categories sc ON sr.category_id = sc.category_id 
              LEFT JOIN users u ON sr.reporter_user_id = u.user_id 
              LEFT JOIN users moderator ON sr.reviewed_by = moderator.user_id
...
<head>
    ...
    <title>Manage Reports - ScamShield Moderator</title>
    ...
    <div class="admin-wrapper">
        ...
        <div class="admin-content">
            ...
            <main class="dashboard-main">
                ...
                <div class="section-card">
                    <h2>All Reports</h2>
                    <p class="info-text">Note: Moderators cannot delete reports. Contact an admin for deletions.</p>
